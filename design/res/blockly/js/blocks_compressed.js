...
Blockly.Constants.Logic.CONTROLS_IF_MUTATOR_MIXIN = {
	...
	reconnectChildBlocks_: function(a, b, c) {
		for (var e = 1; e <= this.elseifCount_; e++) Blockly.Mutator.reconnect(a[e], this, "IF" + e),
		Blockly.Mutator.reconnect(b[e], this, "DO" + e);
...
Blockly.Blocks.procedures_defnoreturn = {
	...
	domToMutation: function(a) {
		...
		c; c = a.childNodes[b]; b++) if ("arg" == c.nodeName.toLowerCase()) {
			...
			c = Blockly.Variables.getOrCreateVariablePackage(this.workspace, c, e, "");
	...
	renameVarById: function(a, b) {
		...
		if ("" == c.type) {
			...
			for (var e = !1,
			d = 0; d < this.argumentVarModels_.length; d++) this.argumentVarModels_[d].getId() == a && (this.arguments_[d] = b.name, this.argumentVarModels_[d] = b, e = !0);
	...
	updateVarName: function(a) {
		for (var b = a.name,
		c = !1,
		e = 0; e < this.argumentVarModels_.length; e++) if (this.argumentVarModels_[e].getId() == a.getId()) {
			var d = this.arguments_[e];
			this.arguments_[e] = b;
			c = !0
		}
		c && (this.displayRenamedVar_(d, b), Blockly.Procedures.mutateCallers(this))
	},
	displayRenamedVar_: function(a, b) {
		this.updateParams_();
		if (this.mutator && this.mutator.isVisible()) for (var c = this.mutator.workspace_.getAllBlocks(!1), e = 0, d; d = c[e]; e++)"procedures_mutatorarg" == d.type && Blockly.Names.equals(a, d.getFieldValue("NAME")) && d.setFieldValue(b, "NAME")
	},
	customContextMenu: function(a) {
		if (!this.isInFlyout) {
			var b = {
			...
			b.text = Blockly.Msg.VARIABLES_SET_CREATE_GET.replace("%1", e.name),
			e = Blockly.Variables.generateVariableFieldDom(e),
...
Blockly.Blocks.procedures_mutatorarg = {
	...
	validator_: function(a) {
		...
		if (!a) return null;
		for (var e = (b.workspace.targetWorkspace || b.workspace).getAllBlocks(!1), d = 0; d < e.length; d++) if (e[d].id != this.getSourceBlock().id && e[d].getFieldValue("NAME") == a) return null;
...
Blockly.Blocks.procedures_callnoreturn = {
	...
	setProcedureParameters_: function(a, b) {
		...
		if (b) if (a.join("\n") == this.arguments_.join("\n")) this.quarkIds_ = b;
			...
			this.argumentVarModels_ = [];
			for (d = 0; d < this.arguments_.length; d++) a = Blockly.Variables.getOrCreateVariablePackage(this.workspace, null, this.arguments_[d], ""),
			this.argumentVarModels_.push(a);
			this.updateShape_();
			if (this.quarkIds_ = b) for (d = 0; d < this.arguments_.length; d++) b = this.quarkIds_[d],
			b in this.quarkConnections_ && (f = this.quarkConnections_[b], Blockly.Mutator.reconnect(f, this, "ARG" + d) || delete this.quarkConnections_[b]); (this.rendered = c) && this.render()
		}
	},
	updateShape_: function() {
...
Blockly.Constants.Text.QUOTE_IMAGE_MIXIN = {
	...
	quoteField_: function(a) {
		for (var b = 0,
		c; c = this.inputList[b]; b++) for (var e = 0,
		d; d = c.fieldRow[e]; e++) if (a == d.name) {
...
Blockly.Constants.VariablesDynamic.CUSTOM_CONTEXT_MENU_VARIABLE_GETTER_SETTER_MIXIN = {
	customContextMenu: function(a) {
		if (!this.isInFlyout) {
			...
			e = Blockly.Msg.VARIABLES_SET_CREATE_GET;
			var d = {
				enabled: 0 < this.workspace.remainingCapacity()
			},
...
Blockly.Blocks.import_game_libs = {
  init: function() {
	  this.appendDummyInput()
      .appendField("导入游戏包");
	  this.setNextStatement(true, null);
	  this.setColour(230);
	  this.setTooltip("导入游戏相关的包");
	  this.setHelpUrl("");
  }
};
...
Blockly.Blocks.create_map = {
  init: function() {
      this.appendDummyInput()
        .appendField("创建游戏地图（20*20）")
        /*.appendField("长")
        .appendField(new Blockly.FieldNumber(8, 1, 19), "DX")
        .appendField("宽")
        .appendField(new Blockly.FieldNumber(8, 1, 19), "DY");*/
      this.appendStatementInput("INIT")
      .setCheck("map");
	  this.setPreviousStatement(true, null);
	  this.setNextStatement(true, "create_map");
	  this.setColour(160);
	  this.setTooltip("创建地图，设备x和y的长度");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.placement_robot = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("放置机器人在：")
	  .appendField("X")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "RDX")
	  .appendField("，")
	  .appendField("Y")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "RDY");
	  this.setPreviousStatement(true, "map");
	  this.setNextStatement(true, null);
	  this.setColour(300);
	  this.setTooltip("放置机器人位置");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.robot_control = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("设置机器人操控：")
	  .appendField("上")
	  .appendField(new Blockly.FieldTextInput("W"), "RUP")
	  .appendField("，")
	  .appendField("下")
	  .appendField(new Blockly.FieldTextInput("S"), "RDOWN")
	  .appendField("，")
	  .appendField("左")
	  .appendField(new Blockly.FieldTextInput("A"), "RLEFT")
	  .appendField("，")
	  .appendField("右")
	  .appendField(new Blockly.FieldTextInput("D"), "RRIGHT");
	  this.setPreviousStatement(true, "map");
	  this.setNextStatement(true, null);
	  this.setColour(300);
	  this.setTooltip("设置机器人操控");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.add_obstacle = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("添加障碍(单个)：")
	  .appendField("X")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "ODX")
	  .appendField("，")
	  .appendField("Y")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "ODY");
	  this.setPreviousStatement(true, "map");
	  this.setNextStatement(true, null);
	  this.setColour(100);
	  this.setTooltip("在指定的位置添加一个障碍");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.add_obstacle_h = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("添加水平障碍（多个）：在")
	  .appendField("Y")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "ODY")
	  .appendField("上，构建X1")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "ODX1")
	  .appendField("到X2")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "ODX2")
	  .appendField("的障碍");
	  this.setPreviousStatement(true, "map");
	  this.setNextStatement(true, null);
	  this.setColour(100);
	  this.setTooltip("在指定的位置添加多个障碍");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.add_obstacle_v = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("添加垂直障碍（多个）：在")
	  .appendField("X")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "ODX")
	  .appendField("上，构建Y1")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "ODY1")
	  .appendField("到Y2")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "ODY2")
	  .appendField("的障碍");
	  this.setPreviousStatement(true, "map");
	  this.setNextStatement(true, null);
	  this.setColour(100);
...
Blockly.Blocks.add_box = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("添加物品：")
	  .appendField("X")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "BDX")
	  .appendField("，")
	  .appendField("Y")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "BDY");
	  this.setPreviousStatement(true, "map");
	  this.setNextStatement(true, null);
	  this.setColour(200);
	  this.setTooltip("在指定的位置添加一个物品");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.add_target = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("添加目标位置：")
	  .appendField("X")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "TDX")
	  .appendField("，")
	  .appendField("Y")
	  .appendField(new Blockly.FieldNumber(0, 0, 19), "TDY");
	  this.setPreviousStatement(true, "map");
	  this.setNextStatement(true, null);
	  this.setColour(200);
	  this.setTooltip("在指定的位置添加一个目标位置");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.import_screen_libs = {
  init: function() {
	  this.appendDummyInput()
      .appendField("导入像素屏包");
	  this.setNextStatement(true, null);
	  this.setColour(230);
	  this.setTooltip("导入像素屏包");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.open_serial_port = {
  init: function() {
  	var a = [["9600", "9600"], ["19200", "19200"], ["38400", "38400"], ["57600", "57600"], ["115200", "115200"]];
	  this.appendDummyInput()
	  .appendField("连接串口：")
	  .appendField(new Blockly.FieldTextInput("COM1"), "SP")
	  .appendField("，并设置波特率：")
	  .appendField(new Blockly.FieldDropdown(a), "BR");
	  this.setPreviousStatement(true, null);
	  this.setNextStatement(true, null);
	  this.setColour(300);
	  this.setTooltip("连接串口");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.close_serial_port = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("断开串口")
	  this.setPreviousStatement(true, null);
	  this.setNextStatement(true, null);
	  this.setColour(300);
	  this.setTooltip("断开串口");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.waitting = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("等待")
	  .appendField(new Blockly.FieldNumber(1000, 1, 100000), "MS")
	  .appendField("毫秒");
	  this.setPreviousStatement(true, null);
	  this.setNextStatement(true, null);
	  this.setColour(200);
	  this.setTooltip("等待多少毫秒");
	  this.setHelpUrl("");
  }
};
...
Blockly.Blocks.end_and_display = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("结束并显示")
	  this.setPreviousStatement(true, null);
	  this.setNextStatement(true, null);
	  this.setColour(100);
	  this.setTooltip("结束并显示");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.get_time_str = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("取得当前时间字符串");
	  this.setOutput(true, null);
	  this.setColour(170);
	  this.setTooltip("取得当前时间字符串");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.out_pixel = {
  init: function() {
	  this.appendDummyInput()
	  .appendField("输出一个点在坐标X")
	  .appendField(new Blockly.FieldNumber(0, 0, 100), "SDX")
	  .appendField("Y")
	  .appendField(new Blockly.FieldNumber(1, 0, 100), "SDY");
...
Blockly.Blocks.out_str = {
  init: function() {
	  ...
	  .appendField(new Blockly.FieldNumber(0, 0, 100), "SDX")
	  .appendField("Y")
	  .appendField(new Blockly.FieldNumber(6, 0, 100), "SDY");
	  this.appendValueInput("SCOLOR")
        .setCheck(null)
        .appendField("颜色为");
	  this.setPreviousStatement(true, null);
	  this.setNextStatement(true, null);
	  this.setColour(100);
	  this.setTooltip("输出文本到串口");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.out_vline = {
  init: function() {
  	this.appendDummyInput()
	  .appendField("输出以坐标X")
	  .appendField(new Blockly.FieldNumber(0, 0, 100), "SDX")
	  ...
	  .appendField(new Blockly.FieldNumber(1, 0, 100), "SDY")
	  .appendField("为原点，长度为")
	  .appendField(new Blockly.FieldNumber(1, 1, 255), "SL")
	  .appendField("的垂直线，");
	  this.appendValueInput("SCOLOR")
        .setCheck(null)
        .appendField("颜色为");
    this.setInputsInline(true);
	  ...
	  this.setNextStatement(true, null);
	  this.setColour(100);
	  this.setTooltip("输出一条垂直线到串口");
	  this.setHelpUrl("");
  }
};

Blockly.Blocks.out_hline = {
  init: function() {
  	this.appendDummyInput()
	  .appendField("输出以坐标X")
	  .appendField(new Blockly.FieldNumber(0, 0, 100), "SDX")
